    ...
    kotlin {
        android {
            optimization.consumerKeepRules.apply {
                publish = true
                file("proguard-rules-consumer.pro")
            }
        }
    }
    ```
...
2. **Move Android entry point code** from `src/androidMain/` to `androidApp/src/main/`:
   ...
   - `AndroidManifest.xml` (app-level manifest with `<application>` and launcher `<activity>`)
...
7. **Remove legacy workarounds** from `gradle.properties`:
   - Remove `android.enableLegacyVariantApi=true`
   ...
   - Remove `android.r8.integratedResourceShrinking` (causes error in AGP 9.0)
   - Remove `android.enableNewResourceShrinker.preciseShrinking` (causes error in AGP 9.0)
...
**Warning:** These flags will be removed in AGP 10.0. Ask the user if they want to opt out, and if so,
...
2. **Check each against the tables above** and the live tracker at https://agp-status.frybits.com/agp-9.0.0/
...
- `android.enableLegacyVariantApi` — removed in AGP 9.0
...
- **Consumer ProGuard rules silently dropped** without `publish = true`
- **Hilt/KSP** may lack AGP 9.0 support — check [compatibility tracker](https://agp-status.frybits.com/agp-9.0.0/)
