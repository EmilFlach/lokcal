...
1. **Framework import:** Change `import ComposeApp` to `import Shared` in all Swift files.

2. **Gradle task path:** Update the build phase script:
   ```bash
...
3. **Framework search paths:** Update Build Settings if they reference the old module name.

4. **Cocoapods (if used):** Update the pod spec name:
   ```kotlin
   // shared/build.gradle.kts
   kotlin {
       cocoapods {
