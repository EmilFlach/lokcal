...
1. Update the framework search path in Xcode build settings
2. Update the `import ComposeApp` to `import Shared` (matching `baseName` in framework config)
3. Update the Run Script phase that invokes `embedAndSignAppleFrameworkForXcode` to reference the new module path
