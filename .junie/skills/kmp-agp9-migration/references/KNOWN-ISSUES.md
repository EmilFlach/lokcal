...
- **BuildKonfig** — better flavor system (similar to Android build variants), simpler API, purpose-built for KMP
- **gradle-buildconfig-plugin** — richer type support (arrays, maps, enums), works in non-KMP projects too, more actively maintained
...
**Problem:** Hilt requires the Android Gradle plugin's variant-aware transform pipeline and annotation processing, which is not available in the KMP library plugin. Hilt cannot be used in modules using `com.android.kotlin.multiplatform.library`.

**Impact:** High for projects using Hilt extensively.

**Workaround:**

- Use [kotlin-inject](https://github.com/evant/kotlin-inject) or [Koin](https://insert-koin.io/) for KMP-compatible DI.
- Keep Hilt usage in the Android application module only.
- Define interfaces in the shared module, implement and provide them via Hilt in the app module.
...
## 5. KSP Compatibility Gap

**Problem:** KSP (Kotlin Symbol Processing) support with the KMP library plugin may have version compatibility issues. Some KSP processors do not yet support the new plugin structure.

**Impact:** Medium. Affects projects using Room, Moshi code-gen, or other KSP-based libraries.

**Workaround:**

- Ensure KSP version matches your KGP version (e.g., KSP 2.3.0-1.0.x for KGP 2.3.0).
- Use KSP's multiplatform configuration:
  ```kotlin
  plugins {
  ...
  ```
- If a specific processor fails, check its issue tracker for KMP library plugin support.

---

## 6. Compose Resources Require Explicit Enable
...
## 7. Consumer ProGuard Rules Silently Dropped
...
## 8. Convention Plugin Refactoring Needed
...
## 9. androidUnitTest to androidHostTest Rename
...
## 11. Packaging Exclusions Syntax Change
...
## 12. Static BuildConfig.DEBUG for Tree-Shaking No Longer Available
...
## 13. android.enableLegacyVariantApi=true (Temporary Escape Hatch)

**Problem:** Some build scripts or plugins depend on the legacy variant API (`androidComponents { onVariants { ... } }`), which is unavailable or limited in the KMP library plugin.
...
**Warning:** This flag is deprecated and will be removed in AGP 10. It exists only as a transitional aid. Do not depend on it long-term. Plan to remove usage before AGP 10.

---

## 14. android.builtInKotlin=false (Temporary Opt-Out)
...
## 15. IDE Support Matrix

| IDE | KMP Library Plugin Support | Notes |
|---|---|---|
| Android Studio Otter 3 FD (2025.2.3) | Full | First version with full AGP 9.0 support |
...
| IntelliJ IDEA | Not supported | Does not support AGP 9.0 as of 2026.1; use Android Studio |
| Fleet | Experimental | Limited support, check JetBrains roadmap |

### Recommended IDE Setup

- **Use Android Studio Otter 3 FD or later.** This is currently the only IDE with full AGP 9.0 + KMP library plugin support.
...
- Invalidate caches and restart after migration (`File > Invalidate Caches`).

---

## 16. kapt Incompatible with Built-in Kotlin
...
**Impact:** High. Many projects use kapt for Dagger/Hilt, Room, Moshi, etc.
...
## 17. New DSL Interfaces (BaseExtension Removed)
...
## 18. Deprecated Variant APIs Removed
...
## 19. R8 and ProGuard Rule Changes
...
## 21. R Class Non-Final in Application Modules
...
## 22. targetSdk Defaults to compileSdk
...
## 23. Third-Party Plugin Compatibility
...
**Key broken plugins with no workaround:**
...
**Live tracker:** https://agp-status.frybits.com/agp-9.0.0/

---

## Summary Table

| # | Issue | Severity | Has Workaround |
|---|---|---|---|
| 1 | BuildConfig removed | High | Yes (AppConfiguration DI) |
| 2 | NDK/JNI unsupported | Medium | Yes (proxy module) |
| 3 | No build variants | High | Yes (runtime config) |
| 4 | Hilt unsupported | High | Yes (alternative DI) |
| 5 | KSP compatibility | Medium | Partial |
| 6 | Resources need explicit enable | High | Yes (one-line fix) |
| 7 | ProGuard rules silently dropped | Medium | Yes (DSL migration) |
| 8 | Convention plugin refactoring | Medium-High | Yes (rewrite) |
| 9 | Test source set rename | Low | Yes (rename dirs) |
| 10 | useK2Uast deprecated | Low | Yes (remove line) |
| 11 | Packaging exclusions syntax | Low | Yes (explicit entries) |
| 12 | No static DEBUG for tree-shaking | Low-Medium | Yes (R8 rules) |
| 13 | Legacy variant API escape hatch | Medium | Temporary flag |
| 14 | Built-in Kotlin opt-out | Low | Temporary flag |
| 15 | IDE support varies | Medium | Use recommended IDE |
| 16 | kapt incompatible with built-in Kotlin | High | Yes (KSP or legacy-kapt) |
| 17 | BaseExtension removed (new DSL) | High | Temporary opt-out |
| 18 | Deprecated variant APIs removed | Medium-High | Yes (androidComponents API) |
| 19 | R8/ProGuard rule changes | Medium | Yes (update rules) |
| 20 | Removed features (Wear, density splits) | Low-Medium | Varies |
| 21 | R class non-final in apps | Medium | Yes (if/else refactor) |
| 22 | targetSdk defaults to compileSdk | Medium | Yes (set explicitly) |
| 23 | Third-party plugin compatibility | High | Varies (see tracker) |
