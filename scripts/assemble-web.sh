...
#
# Output: build/web-dist/  (serve it with any static server; see scripts/serve-web.sh)
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "[6/6] Serve it with:  scripts/serve-web.sh   (or: cd ${OUT} && python3 -m http.server 8099)"
