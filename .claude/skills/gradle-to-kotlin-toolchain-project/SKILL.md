...
description: Migrate an entire Gradle Kotlin/JVM project to Kotlin Toolchain (Amper), including module.yaml and project.yaml, dependency catalogs, CI commands, and local plugins replacing unsupported Gradle functionality. Use when switching a whole Gradle build to Toolchain, including application packaging, versioning, containers, linting, resources, and code generation. For a single plugin port use gradle-to-kotlin-toolchain-plugin; for a new Toolchain plugin use kotlin-toolchain-plugin-authoring; for an existing Toolchain project use kotlin-toolchain.
---
